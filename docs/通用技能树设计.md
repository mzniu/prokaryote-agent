...
通用技能树能够随着Agent的进化而**自动扩展**，这是与固定领域技能树的核心区别。

#### 4.4.1 触发条件

| 触发类型 | 条件 | 说明 |
|---------|------|------|
| 里程碑触发 | 每5次成功进化 | 定期检查优化机会 |
| 等级触发 | 通用技能达到5/10/15/20级 | 关键节点深度分析 |
| 能力触发 | 总等级 ≥ 10 | 具备最小能力基础 |
...
```json
{
  ...
  "max_level": 20,
  "description": "控制浏览器执行复杂交互操作",
  "capabilities": ["页面导航", "表单填充", "截图"],
  "prerequisites": ["web_scraping_advanced"],
  "unlock_condition": "web_scraping_advanced >= 5",
  "unlocked": false,
  "ai_generated": true,
  "generated_at": "2026-02-07T16:00:00"
}
```
...
1. **数量限制**：每次优化最多添加2个新技能
2. **去重检查**：不添加已存在或相似的技能
3. **依赖验证**：新技能必须有明确的前置技能
4. **类别平衡**：优先补充最弱的类别
  ...
  "optimization_history": [
    {
      ...
      "trigger_skill": "web_scraping_basic",
      "trigger_level": 5,
      "changes": [
        {
          "type": "add_skill",
          "skill_id": "browser_automation",
          "reason": "与网页抓取能力协同，扩展交互能力"
        }
      ]
    }
  ],
  "last_optimized": "2026-02-07T16:00:00"
}
```
